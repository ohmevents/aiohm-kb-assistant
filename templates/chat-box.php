...
class AIOHM_KB_Chat_Box {
    ...
    public static function render_message($content, $type = 'user', $timestamp = null, $metadata = array()) {
        ...
        $classes = array('aiohm-message', 'aiohm-message-' . $type);
        ...
        if ($type === 'bot' && !empty($metadata['sources'])) {
            ...
            $output .= __('Sources', 'aiohm-kb-assistant') . ' (' . count($metadata['sources']) . ')';
            $output .= '</button>';
            ...
            foreach ($metadata['sources'] as $source) {
                ...
                if (!empty($source['metadata']['url'])) {
                    ...
                    $output .= __('View Source', 'aiohm-kb-assistant');
                    $output .= '</a>';
                }
                $output .= '</div>';
            }
    ...
    public static function render_suggested_questions() {
        $suggestions = array(
            __('What services do you offer?', 'aiohm-kb-assistant'),
            __('How can I contact support?', 'aiohm-kb-assistant'),
            __('What are your business hours?', 'aiohm-kb-assistant'),
            __('Do you have a FAQ section?', 'aiohm-kb-assistant')
        );
        ...
        $output .= '<div class="aiohm-suggestions-title">' . __('Suggested questions:', 'aiohm-kb-assistant') . '</div>';
    ...
    public static function render_chat_input($placeholder = '', $features = array()) {
        if (empty($placeholder)) {
            $placeholder = __('Type your message...', 'aiohm-kb-assistant');
        }
        ...
        if ($features['show_emoji'] || $features['show_attachment'] || $features['show_voice']) {
            ...
            if ($features['show_emoji']) {
                $output .= '<button type="button" class="aiohm-feature-btn aiohm-emoji-btn" title="' . __('Add emoji', 'aiohm-kb-assistant') . '">';
                $output .= '😊';
                $output .= '</button>';
            }
            
            if ($features['show_attachment']) {
                $output .= '<button type="button" class="aiohm-feature-btn aiohm-attachment-btn" title="' . __('Attach file', 'aiohm-kb-assistant') . '">';
        ...
        if ($features['show_voice']) {
            $output .= '<button type="button" class="aiohm-feature-btn aiohm-voice-btn" title="' . __('Voice input', 'aiohm-kb-assistant') . '">';
            $output .= '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">';
